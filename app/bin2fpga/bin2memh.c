...
int main(int argc, char** argv)
{
    ...
    if (argc != 3) {
        ...
        fprintf(stderr, "\nUsage: bin2fpga Depth InputFile\n");
        return 0;
    }
    else {
        ...
        // Xilinx no address in the hex file
        ...
        // Xilinx size of the initialization data must exactly match the size
        for (; i < maxaddr; i += 4) {
            fprintf(stdout, "@%08x\n", address);

            fprintf(stdout, "%02x", 0x0);
            fprintf(stdout, "%02x", 0x0);
            fprintf(stdout, "%02x", 0x0);
            fprintf(stdout, "%02x", 0x0);
            fprintf(stdout, "\n");
